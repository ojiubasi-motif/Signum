...
exports.processMemberMessage = processMemberMessage;
...
function getGroq() {
    if (!groqInstance) {
        groqInstance = new groq_sdk_1.default({
 ...
 */
async function processMemberMessage(from, text) {
    ...
    if (normalizedText === 'active' || normalizedText === 'open') {
        ...
        reply += signals
            ...
            `└─ Posted By: Admin *${s.admin.name === s.admin.id.split('@')[0] ? (0, formatter_1.formatWhatsappNumber)(s.admin.id) : s.admin.name}*`)
            .join('\n\n');
        return reply;
    }
    // 1.5 EXPIRED / CLOSED SIGNALS QUERY (cached in Redis)
    if (normalizedText === 'expired' || normalizedText === 'history' || normalizedText === 'closed') {
        const cachedSignals = await (0, cache_1.getCachedResolvedSignals)();
    ...
    if (normalizedText === 'stats' || normalizedText === 'win rate' || normalizedText === 'leaderboard') {
        ...
        reply += admins
            .map(a => `👤 *Admin ${a.name === a.id.split('@')[0] ? (0, formatter_1.formatWhatsappNumber)(a.id) : a.name}*\n` +
    ...
    if (mockMatch) {
        ...
        (0, binance_1.setMockPrice)(assetName, priceValue);
    ...
    if (tradeMatch) {
        ...
        const signal = await index_1.prisma.signal.findFirst({
            where: {
                asset: assetName,
                status: 'ENTRY_OPEN',
            },
        ...
        await index_1.prisma.memberTrade.create({
            data: {
                memberId: member.id,
                signalId: signal.id,
            },
        });
        return (`✅ *Trade Registered!* 🚀\n\n` +
    ...
    try {
        ...
        const resolvedSignals = await (0, cache_1.getCachedResolvedSignals)();
        const systemPrompt = 'You are Signum, a helpful conversational AI assistant representing the Signum autonomous crypto trading signal bot. ' +
